...
void setup() {
  ...
  //--------------- DISTANCIA-------
  pinMode(13, OUTPUT); //salida del pulso generado por el sensor ultrasónico 10-13
  pinMode(8, INPUT);//entrada del pulso generado por el sensor ultrasónico 9-10
  pinMode(9, OUTPUT);//alarma de la distancia(encenderá el led) 11-9
...
void loop() {
  if (Serial.available() > 0) {
    ...
    //------------- DISTANCIA ------------
      ...
      delayMicroseconds(8);
      tiem=pulseIn(8, HIGH); //fórmula para medir el pulso entrante.
      dis=  int(0.017 * tiem); //fórmula para calcular la distancia del sensor ultrasónico.
